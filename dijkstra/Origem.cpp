...
#define MAX 10 // usado para definir o tamanho do vetor de distancias do Djikstra
...
void dijkstra(struct Grafo* Grafo, int VerticeOrigem)
{
    ...
    struct ListaDeVizinhos* ElementoVarredura;
    while (TotalVisitados < (Grafo->TotalVertices - 1))
    {
        minDistancia = INT_MAX;
        // ProxVertice eh nodo com a menor distancia
        for (i = 0; i < Grafo->TotalVertices; i++)
        {
    ...
    // print do caminho e  Distancia de cada nodo
    for (i = 0; i < Grafo->TotalVertices; i++)
    {
        if (i != VerticeOrigem)
        {
